...
func DeleteCmd() command.SetupCommand[*config.Config] {
	return func(cmd *cobra.Command, ch *cmdutils.Helper[*config.Config]) {
		deleteCmd := &cobra.Command{
			...
			Short: "delete a Server Template with the given identifier and authority",
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				end := ch.Printer.PrintProgress(fmt.Sprintf("Deleting Server Template %s for authority %s...", identifier, authority))
				...
				if ch.Printer.Format() == printer.Human {
					ch.Printer.Printf("%s %s %s\n", printer.BoldRed("Server Template"), printer.BoldGreen(identifier), printer.BoldRed("deleted"))
					return nil
				}
