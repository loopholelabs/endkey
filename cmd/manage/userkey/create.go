...
// CreateCmd encapsulates the commands for creating User Keys
func CreateCmd() command.SetupCommand[*config.Config] {
	return func(cmd *cobra.Command, ch *cmdutils.Helper[*config.Config]) {
		createCmd := &cobra.Command{
			Use:   "create <name>",
			Args:  cobra.ExactArgs(1),
			Short: "Create an User Key with the given name",
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				end := ch.Printer.PrintProgress(fmt.Sprintf("Creating User Key %s...", name))
				res, err := client.Userkey.PostUserkeyName(userkey.NewPostUserkeyNameParamsWithContext(ctx).WithName(name))
				end()
				if err != nil {
					return err
				}
				...
				if ch.Printer.Format() == printer.Human {
					ch.Printer.Printf("Created User Key '%s': %s (this will only be displayed once)\n", printer.Bold(res.Payload.Name), printer.BoldGreen(value))
					return nil
				}
