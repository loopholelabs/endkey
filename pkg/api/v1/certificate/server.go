...
func (a *Certificate) CreateServer(ctx *fiber.Ctx) error {
	...
	if err != nil {
		...
		templ, err = a.options.Database().GetServerTemplateByAPIKey(ctx.Context(), body.TemplateName, ak)
		if err != nil {
	...
	a.logger.Info().Msgf("creating server certificate for template '%s' with authority '%s', for api key %s", body.TemplateName, auth.Name, ak.Name)
