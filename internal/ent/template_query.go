...
func (tq *TemplateQuery) QueryAuthority() *AuthorityQuery {
	query := (&AuthorityClient{config: tq.config}).Query()
...
func (tq *TemplateQuery) QueryAPIKeys() *APIKeyQuery {
...
func (tq *TemplateQuery) Clone() *TemplateQuery {
	...
	return &TemplateQuery{
		...
		withAuthority: tq.withAuthority.Clone(),
		withAPIKeys:   tq.withAPIKeys.Clone(),
...
func (tq *TemplateQuery) WithAuthority(opts ...func(*AuthorityQuery)) *TemplateQuery {
	query := (&AuthorityClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withAuthority = query
	return tq
}
...
func (tq *TemplateQuery) WithAPIKeys(opts ...func(*APIKeyQuery)) *TemplateQuery {
	query := (&APIKeyClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withAPIKeys = query
	return tq
}
...
func (tq *TemplateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Template, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [2]bool{
			tq.withAuthority != nil,
			tq.withAPIKeys != nil,
		}
	)
	if tq.withAuthority != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withAuthority; query != nil {
		if err := tq.loadAuthority(ctx, query, nodes, nil,
			func(n *Template, e *Authority) { n.Edges.Authority = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withAPIKeys; query != nil {
		if err := tq.loadAPIKeys(ctx, query, nodes,
			func(n *Template) { n.Edges.APIKeys = []*APIKey{} },
			func(n *Template, e *APIKey) { n.Edges.APIKeys = append(n.Edges.APIKeys, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TemplateQuery) loadAuthority(ctx context.Context, query *AuthorityQuery, nodes []*Template, init func(*Template), assign func(*Template, *Authority)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Template)
	for i := range nodes {
		if nodes[i].authority_templates == nil {
			continue
		}
		fk := *nodes[i].authority_templates
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "authority_templates" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TemplateQuery) loadAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Template, init func(*Template), assign func(*Template, *APIKey)) error {
	...
	query.Where(predicate.APIKey(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(template.APIKeysColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.template_api_keys
		if fk == nil {
			return fmt.Errorf(`foreign-key "template_api_keys" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "template_api_keys" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
