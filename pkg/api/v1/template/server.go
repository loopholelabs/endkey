...
func (a *Template) CreateServer(ctx *fiber.Ctx) error {
	...
	a.logger.Info().Msgf("creating server template '%s' with authority '%s', common name '%s', dns names '%v', and a validity of %s for user key %s", body.Name, body.AuthorityName, body.CommonName, body.DNSNames, validity.String(), uk.Name)

	templ, err := a.options.Database().CreateServerTemplate(ctx.Context(), body.Name, body.AuthorityName, uk, body.CommonName, body.Tag, body.DNSNames, body.AllowAdditionalDNSNames, body.IPAddresses, body.AllowAdditionalIPs, validity.String())
	if err != nil {
...
// @Success      200  {array} models.ServerTemplateResponse
// @Failure      400  {string} string
// @Failure      401  {string} string
// @Failure      404  {string} string
// @Failure      409  {string} string
// @Failure      412  {string} string
// @Failure      500  {string} string
// @Router       /template/server/{authority_name} [get]
func (a *Template) ListServer(ctx *fiber.Ctx) error {
	a.logger.Debug().Msgf("received ListServer request from %s", ctx.IP())
	...
	a.logger.Info().Msgf("listing server templates for authority '%s' for user key %s", authorityName, uk.Name)

	templs, err := a.options.Database().ListServerTemplates(ctx.Context(), authorityName, uk)
	if err != nil {
		if errors.Is(err, database.ErrNotFound) {
			return fiber.NewError(fiber.StatusNotFound, "no server templates found")
		}

		a.logger.Error().Err(err).Msg("failed to list server templates")
		return fiber.NewError(fiber.StatusInternalServerError, "failed to list server templates")
	}

	ret := make([]*models.ServerTemplateResponse, 0, len(templs))
	for _, templ := range templs {
		ret = append(ret, &models.ServerTemplateResponse{
			CreatedAt:               templ.CreatedAt.Format(time.RFC3339),
			ID:                      templ.ID,
			Name:                    templ.Name,
...
// @Accept       json
// @Produce      json
// @Param        request  body models.DeleteServerTemplateRequest  true  "Delete Server Template Request"
...
// @Failure      412  {string} string
// @Failure      500  {string} string
// @Router       /template/server [delete]
func (a *Template) DeleteServer(ctx *fiber.Ctx) error {
	a.logger.Debug().Msgf("received DeleteServer request from %s", ctx.IP())
	...
	body := new(models.DeleteServerTemplateRequest)
	err = ctx.BodyParser(body)
	if err != nil {
		a.logger.Error().Err(err).Msg("failed to parse body")
		return fiber.NewError(fiber.StatusBadRequest, "failed to parse body")
	...
	a.logger.Info().Msgf("deleting server template '%s' with authority '%s' for user key %s", body.Name, body.AuthorityName, uk.Name)

	err = a.options.Database().DeleteServerTemplateByName(ctx.Context(), body.Name, body.AuthorityName, uk)
	if err != nil {
		if errors.Is(err, database.ErrNotFound) {
			return fiber.NewError(fiber.StatusNotFound, "server template not found")
		}

		if errors.Is(err, database.ErrAlreadyExists) {
			return fiber.NewError(fiber.StatusConflict, "cannot delete server template because resources are still associated with it")
		}

		a.logger.Error().Err(err).Msg("failed to delete server template")
		return fiber.NewError(fiber.StatusInternalServerError, "failed to delete server template")
	}

	return ctx.SendString(fmt.Sprintf("server template '%s' for authority '%s' deleted", body.Name, body.AuthorityName))
}
