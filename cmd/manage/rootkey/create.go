...
// CreateCmd encapsulates the commands for creating Root Keys
func CreateCmd() command.SetupCommand[*config.Config] {
	return func(cmd *cobra.Command, ch *cmdutils.Helper[*config.Config]) {
		createCmd := &cobra.Command{
			Use:   "create <name>",
			Args:  cobra.ExactArgs(1),
			Short: "Create an Root Key with the given name",
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				end := ch.Printer.PrintProgress(fmt.Sprintf("Creating Root Key %s...", name))
				res, err := client.Rootkey.PostRootkeyName(rootkey.NewPostRootkeyNameParamsWithContext(ctx).WithName(name))
				end()
				if err != nil {
					return err
				}
