...
func (a *Template) CreateServer(ctx *fiber.Ctx) error {
	...
	templ, err := a.options.Database().CreateServerTemplate(ctx.Context(), body.Identifier, body.Authority, body.CommonName, body.Tag, body.DNSNames, body.AllowAdditionalDNSNames, body.IPAddresses, body.AllowAdditionalIPs, validity.String())
	if err != nil {
...
// @Failure      412  {string} string
// @Failure      500  {string} string
// @Router       /template/server/{authority} [get]
func (a *Template) ListServer(ctx *fiber.Ctx) error {
	...
	a.logger.Info().Msgf("listing Server Templates for Authority '%s' for root key with ID %s", authority, rk.Identifier)

	templs, err := a.options.Database().ListServerTemplates(ctx.Context(), authority)
	if err != nil {
...
func (a *Template) DeleteServer(ctx *fiber.Ctx) error {
	...
	a.logger.Info().Msgf("deleting server template '%s' with authority '%s' for root key with ID %s", body.Identifier, body.Authority, rk.Identifier)

	err = a.options.Database().DeleteServerTemplate(ctx.Context(), body.Identifier, body.Authority)
	if err != nil {
		if errors.Is(err, database.ErrNotFound) {
	...
	return ctx.SendString(fmt.Sprintf("server template '%s' for authority '%s' deleted", body.Identifier, body.Authority))
}
