...
// The AuthorityFunc type is an adapter to allow the use of ordinary
// function as Authority mutator.
type AuthorityFunc func(context.Context, *ent.AuthorityMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AuthorityFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AuthorityMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f RootKeyFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RootKeyMutation); ok {
		return f(ctx, mv)
	}
